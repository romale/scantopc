...
const (
	...
	RegisterTimeOut = 30 * time.Minute  // Give register interval
)
...
func (d *Device) MainLoop() error {
	...
	if err == nil {
		err = d.StartEventChannel()
	}
	if err == nil {
		d.register = time.After(RegisterTimeOut)
		for {
			select {
...
func (d *Device) StartEventChannel() error {
